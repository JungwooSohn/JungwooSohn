...
/* 원형 */
void read_cards(int hands[][2]);
void analyze_hand(int hands[][2]);
...
void analyze_hand(int hands[][2])
{
    ...
    /* 플러시인지 확인 */
    for (suit = 0; suit < NUM_SUITS - 1; ++suit) {
        if (hands[suit][1] == hands[suit + 1][1]) {
            g_is_flush = true;
        }
    }
    ...
    for(; rank < NUM_RANKS - 1; ++rank) {
        if (abs(hands[rank][0] - hands[rank + 1][0]) == 1)
        {
            ++num_consec;
        }
    }
    ...
    while (card < NUM_CARDS) {
        ...
        switch (run) {
        case 2: g_pairs++;      break;
        case 3: g_is_three = true; break;
        case 4: g_is_four = true;  break;
        }
    }
}
